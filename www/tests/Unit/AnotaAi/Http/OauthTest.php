...
beforeEach(function () {
    ...
    $this->token = fake()->sha256();
    $this->hashRedis = md5($this->merchantId . $this->token);
...
test('obtém cabeçalhos', function () {
    ...
    $oauth = new Oauth($this->redisClient, $this->merchantId, $this->token);
...
test('exceção resposta não HTTP_OK', function () {
    ...
    new Oauth($this->redisClient, $this->merchantId, $this->token);
});
