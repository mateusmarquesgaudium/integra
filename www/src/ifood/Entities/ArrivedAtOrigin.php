...
class ArrivedAtOrigin extends EventsIfoodManager
{
    ...
    {
        $keyEventRateLimit = str_replace('{event_type}', 'arrivedAtOrigin', RedisSchema::KEY_RATE_LIMIT_ORDERS_EVENTS);
        $maxRequests = 3000;
        ...
        $request = new Request("{$this->customIfood['uri']}/logistics/v1.0/orders/{$event['orderId']}/arrivedAtOrigin");
        $response = $request
            ->setHeaders([
                'Content-Length: 0',
