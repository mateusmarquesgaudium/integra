...
class OrderInTransit extends DeliveryOrderInTransit
{
    ...
    public function send(array &$event): bool
    {
        ...
        $url = $this->custom['url'] . '/orders/mark-as-ready';
        ...
        if ($response->http_code === RequestConstants::HTTP_UNAUTHORIZED) {
            return $this->handleUnauthorizedResponse($event);
        }

        return in_array($response->http_code, [RequestConstants::HTTP_OK, RequestConstants::HTTP_NO_CONTENT]);
    }

    private function handleUnauthorizedResponse(array &$event): bool
    {
        $event['timeToRetry'] = time() + $this->custom['timeToRetry'];
        $this->redisService->sAdd(RedisSchema::KEY_AUTHENTICATE_AIQFOME_REFRESH, $event['merchantId']);
        return false;
    }
}