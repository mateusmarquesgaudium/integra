...
$redis->checkMonitor(IzaVariables::KEY_MONITOR_FINISH_STOP_PERIOD, $maxInstances);
$redis->incrMonitor(IzaVariables::KEY_MONITOR_FINISH_STOP_PERIOD);
...
$events = $redis->lRange(IzaVariables::KEY_EVENTS_FINISH_STOP_PERIOD, 0, $maxEventsAtATime - 1);
if (!empty($events) && is_array($events)) {
    $totalEvents = count($events);
    $redis->ltrim(IzaVariables::KEY_EVENTS_FINISH_STOP_PERIOD, $totalEvents, -1);
    ...
    foreach ($events as $key => &$event) {
        ...
        if (!empty($event['last_attempt']) && strtotime('now') <= strtotime($event['last_attempt'] . ' +' . $intervalEventsRetry . ' minutes')) {
            $pipelineRetry[] = ['rPush', IzaVariables::KEY_EVENTS_FINISH_STOP_PERIOD, json_encode($event)];
            $totalEventsNotRetry++;
            unset($events[$key]);
            continue;
        }
    ...
    if ($requestMulti->haveRequests()) {
        ...
        foreach ($events as $eventResult) {
            ...
            $finishedAt = !empty($content['finished_at']) ? (new DateTime($content['finished_at']))->format('Y-m-d H:i:s') : null;

            if (!empty($request) && in_array($httpCode, [RequestConstants::HTTP_OK, RequestConstants::HTTP_CREATED, RequestConstants::HTTP_ACCEPTED])
                && (!empty($content['status']) && $content['status'] == 'active' || !empty($finishedAt) && strtotime($finishedAt) == strtotime($eventResult['data_hora']))
                ...
                $pipeline[] = ['lPush', IzaVariables::KEY_EVENTS_ERROR_FINISH_STOP_PERIOD, json_encode($eventResult)];
            } else {
...
$redis->decr(IzaVariables::KEY_MONITOR_FINISH_STOP_PERIOD);
