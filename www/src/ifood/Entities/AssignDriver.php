...
class AssignDriver extends EventsIfoodManager
{
    ...
    {
        $keyEventRateLimit = str_replace('{event_type}', 'assignDriver', RedisSchema::KEY_RATE_LIMIT_ORDERS_EVENTS);
        $maxRequests = 3000;
        ...
        $bodyRequest = [
            ...
            'workerVehicleType' => $event['workerVehicleType'],
        ];
        $request = new Request("{$this->customIfood['uri']}/logistics/v1.0/orders/{$event['orderId']}/assignDriver");
        $response = $request
            ->setHeaders($this->oauth->getHeadersRequests())
            ->setPostFields($bodyRequest)
