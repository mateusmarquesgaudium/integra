...
$redis->checkMonitor(IzaVariables::KEY_MONITOR_CANCEL_PERIOD, $maxInstances);
$redis->incrMonitor(IzaVariables::KEY_MONITOR_CANCEL_PERIOD);

$maxEventsAtATime = $custom->getIza()['maxEventsAtATime'];
$events = $redis->lRange(IzaVariables::KEY_EVENTS_CANCEL_PERIOD, 0, $maxEventsAtATime - 1);
if (!empty($events) && is_array($events)) {
    $redis->ltrim(IzaVariables::KEY_EVENTS_CANCEL_PERIOD, count($events), -1);
    ...
    foreach ($events as $key => &$event) {
        ...
        if (!empty($event['last_attempt']) && strtotime('now') <= strtotime($event['last_attempt'] . ' +' . $intervalEventsRetry . ' minutes')) {
            $pipelineRetry[] = ['rPush', IzaVariables::KEY_EVENTS_CANCEL_PERIOD, json_encode($event)];
            $totalEventsNotRetry++;
            unset($events[$key]);
            continue;
        }
    ...
    if ($requestMulti->haveRequests()) {
        ...
        foreach ($events as $eventResult) {
            ...
            if (!empty($request) && in_array($httpCode, [RequestConstants::HTTP_OK, RequestConstants::HTTP_CREATED, RequestConstants::HTTP_ACCEPTED]) && !empty($content['status']) && $content['status'] == 'cancelled') {
                ...
                $totalEventsSuccess++;
                $redis->del(IzaVariables::KEY_EVENTS_IN_PROGRESS_PERIOD . $eventResult['index']);
                ...
                $redis->lPush(IzaVariables::KEY_EVENTS_ERROR_CANCEL_PERIOD, json_encode($eventResult));
            } else {
                $eventResult['last_attempt'] = date('Y-m-d H:i:s');
                $redis->rPush(IzaVariables::KEY_EVENTS_CANCEL_PERIOD, json_encode($eventResult));
            }
        }
    }
}
$redis->decr(IzaVariables::KEY_MONITOR_CANCEL_PERIOD);
